    ...
    def get_category_score(self, category: str) -> float:
        ...
        if category == "infrastructure":
            signals = [
                ...
                self.domain_age,
                self.tech_stack_modernity,
                self.uptime_reliability,
            ...
            signals = [
                ...
                self.governance_disclosure,
                self.multilingual_presence,
                self.certification_visibility,
                self.incident_response,
            ]
        elif category == "network":
            signals = [
                self.backlink_quality,
                self.domain_authority,
                self.industry_citations,
                self.partnership_quality,
                self.social_engagement,
                self.supplier_diversity,
            ]
        elif category == "behavioral":
            signals = [
                self.digital_transformation,
                self.innovation_signals,
                self.operational_consistency,
                self.compliance_history,
                self.ir_sophistication,
                self.sustainability_commitment,
            ]
        else:
            return 0.0
...
class DSIEnergyPricingModel:
    ...
    def calculate_confidence(self, signals: DigitalSignals) -> float:
        ...
        all_signals = [
            ...
            signals.domain_age,
            signals.tech_stack_modernity,
            signals.uptime_reliability,
            signals.mobile_optimization,
            signals.update_frequency,
            signals.transparency_score,
            signals.governance_disclosure,
            signals.multilingual_presence,
            signals.certification_visibility,
            signals.incident_response,
            signals.backlink_quality,
            signals.domain_authority,
            signals.industry_citations,
            signals.partnership_quality,
            signals.social_engagement,
            signals.supplier_diversity,
            signals.digital_transformation,
            signals.innovation_signals,
            signals.operational_consistency,
            signals.compliance_history,
            signals.ir_sophistication,
    ...
    def generate_recommendation(
        self, composite_score: float, confidence: float, expected_lr: float
    ) -> Tuple[str, str]:
