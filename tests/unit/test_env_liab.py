"""V6/B4 — env_liab coverage unit tests.
...
EXPECTED_SUBCONFIGS = {'env_industrial', 'env_waste_mgmt', 'env_real_estate', 'env_energy_midstream_xwalk', 'env_sme'}


def test_env_liab_compiles_with_all_sub_configs():
    from infrastructure.models.compiler import get_compiled_configs
    configs = get_compiled_configs()
    cov = configs.get('env_liab')
    assert cov is not None, "env_liab not compiled"
    assert set(cov.configurations.keys()) == EXPECTED_SUBCONFIGS


def test_each_sub_config_has_at_least_22_signals():
    from infrastructure.models.compiler import get_compiled_configs
    cov = get_compiled_configs()['env_liab']
...
def test_inference_functions_registered_for_every_signal():
    ...
    import signal_architecture.signals.inference.functions.env_liab  # noqa: F401

    registered = set(list_inference_functions())
    cfg = get_config('env_liab', 'env_industrial')
    needed = {s.inference_utility_function for s in cfg.signal_registry}
    missing = needed - registered
    assert not missing, f"env_liab inference fns missing: {missing}"
...
def test_primary_sub_config_has_routing_constraint():
    ...
    assert any(
        getattr(rc, "field", None) == 'facility_type'
        for rc in constraints
    ), f"routing on {'facility_type'} missing from env_industrial"
...
def test_end_to_end_assessment_produces_valid_result():
    ...
    config = get_config('env_liab', 'env_industrial')
    engine = get_workflow_engine()
    r = engine.run_workflow(
        entity_id="test-env_liab",
        coverage='env_liab',
        entity_name="ENV_LIAB Test Entity",
        submission_data={
            'revenue': 100000000,
            'limit': 50000000,
            'deductible': 1000000,
            'product_type': 'environmental_impairment',
