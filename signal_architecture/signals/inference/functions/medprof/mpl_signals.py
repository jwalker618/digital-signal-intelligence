"""
MedProf Inference Functions (V6/B1 depth-first build)
=====================================================
...
Generated from coverages/medprof/config.yaml at V6 follow-up time;
signal count should match the config's signal_registry.
"""
...
def _neutral_result(signal_id: str, proxy_tier: str) -> SignalResult:
    """Neutral score (500 of 1000) with proxy-tier-scaled confidence.

    The scoring engine downstream maps confidence into the weighted
    aggregation and raises a MISSING_DATA flag when confidence < 0.3,
    so this scaffold never silently inflates a premium.
    """
    return SignalResult(
        signal_id=signal_id,
        score=50.0,
