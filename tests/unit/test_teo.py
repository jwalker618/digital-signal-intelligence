...
EXPECTED_SUBCONFIGS = {'teo_saas', 'teo_aiml_vendor', 'teo_systems_integrator', 'teo_managed_service_provider', 'teo_sme'}


def test_teo_compiles_with_all_sub_configs():
    from infrastructure.models.compiler import get_compiled_configs
    configs = get_compiled_configs()
    cov = configs.get('teo')
    assert cov is not None, "teo not compiled"
    assert set(cov.configurations.keys()) == EXPECTED_SUBCONFIGS


def test_each_sub_config_has_at_least_22_signals():
    from infrastructure.models.compiler import get_compiled_configs
    cov = get_compiled_configs()['teo']
...
def test_inference_functions_registered_for_every_signal():
    ...
    import signal_architecture.signals.inference.functions.teo  # noqa: F401

    registered = set(list_inference_functions())
    cfg = get_config('teo', 'teo_saas')
    needed = {s.inference_utility_function for s in cfg.signal_registry}
    missing = needed - registered
    assert not missing, f"teo inference fns missing: {missing}"
...
def test_primary_sub_config_has_routing_constraint():
    ...
    assert any(
        getattr(rc, "field", None) == 'business_model'
        for rc in constraints
    ), f"routing on {'business_model'} missing from teo_saas"
...
def test_end_to_end_assessment_produces_valid_result():
    ...
    config = get_config('teo', 'teo_saas')
    engine = get_workflow_engine()
    r = engine.run_workflow(
        entity_id="test-teo",
        coverage='teo',
        entity_name="TEO Test Entity",
        submission_data={
            'revenue': 100000000,
            'limit': 25000000,
            'deductible': 500000,
            'product_type': 'tech_eo',
            'business_model': 'saas',
        },
        config=config,
        skip_discovery=True,
