...
class DSIPortfolioAnalytics:
    ...
    def calculate_portfolio_metrics(self) -> PortfolioMetrics:
        ...
        total_limits = sum(
            getattr(r, "policy_limit", getattr(r, "recommended_limit", 0))
        ...
        # Handle zero premiums case
        if sum(premiums) > 0:
            weighted_avg_score = np.average(scores, weights=premiums)
        else:
            weighted_avg_score = avg_score  # Fallback to simple average
        ...
        for r in self.results:
            if hasattr(r, "expected_loss_ratio"):
                ...
                loss_ratios.append(
                    self._estimate_loss_ratio_from_score(r.composite_score)
                )
        ...
        if sum(premiums) > 0:
            ...
            avg_loss_ratio = np.mean(loss_ratios)  # Fallback to simple average

        # Operational metrics
        auto_approved = sum(
            1 for r in self.results if "auto-approve" in r.recommendation.lower()
        )
    ...
    def _calculate_concentration_score(self) -> float:
        ...
        for p in self.policies:
            ...
            industries[industry] = (
                industries.get(industry, 0) + p["result"].annual_premium
            )

        total_premium = sum(industries.values())

        # Handle zero total premium
        if total_premium == 0:
            return 0
    ...
    def _analyze_dimension(self, dimension: str) -> ConcentrationAnalysis:
        ...
        for policy in self.policies:
            if dimension == "industry":
                key = policy["data"].get(
                    "industry", policy["data"].get("segment", "Unknown")
                )
        ...
        if not recommendations:
            recommendations.append(
                f"{dimension.capitalize()} concentration is well-diversified"
            )
    ...
    def generate_alerts(self) -> List[PortfolioAlert]:
        ...
        if not self.policies:
            return []  # No alerts for empty portfolio
        ...
        for conc in concentrations:
            if conc.risk_level in [
                RiskConcentration.CONCERNING,
                RiskConcentration.CRITICAL,
            ]:
    ...
    def compare_to_benchmark(
        ...
        comparison = {
            "loss_ratio": {
                ...
                "difference": metrics.expected_loss_ratio
                - benchmark.get("loss_ratio", 0.65),
                "better": metrics.expected_loss_ratio
                < benchmark.get("loss_ratio", 0.65),
            },
            "combined_ratio": {
                ...
                "better": metrics.expected_combined_ratio
                < benchmark.get("combined_ratio", 0.98),
            },
            "auto_approval_rate": {
                "portfolio": metrics.auto_approved_pct,
                "benchmark": benchmark.get("auto_approval_rate", 75.0),
                "difference": metrics.auto_approved_pct
                - benchmark.get("auto_approval_rate", 75.0),
                "better": metrics.auto_approved_pct
                > benchmark.get("auto_approval_rate", 75.0),
            },
            "avg_score": {
                "portfolio": metrics.weighted_avg_score,
                "benchmark": benchmark.get("avg_score", 675.0),
                "difference": metrics.weighted_avg_score
                - benchmark.get("avg_score", 675.0),
                "better": metrics.weighted_avg_score
                > benchmark.get("avg_score", 675.0),
            },
        }
    ...
    def export_to_csv(self, filepath: str):
        ...
        for policy in self.policies:
            ...
            row = {
                ...
                "confidence": getattr(r, "confidence_level", 0.0),
            }
            data.append(row)
    ...
    def analyze_concentration(self, dimension: str = "geography") -> ConcentrationAnalysis:
        """Analyze portfolio concentration by specified dimension"""
        if not self.policies:
            return ConcentrationAnalysis(
                concentration_type=dimension,
                concentrations={},
        ...
        concentrations = {}
        for policy in self.policies:
            # Map dimensions to actual policy data fields
            dimension_map = {
                "geography": "country",
                "coverage": "model_type",
                "industry": "industry"
            }
            field_name = dimension_map.get(dimension, dimension)
            key = policy["data"].get(field_name, "unknown")
            concentrations[key] = concentrations.get(key, 0) + 1

        total = len(self.policies)
        percentages = {k: {"count": v, "percentage": (v / total) * 100} for k, v in concentrations.items()}
        max_concentration = max(p["percentage"] for p in percentages.values()) if percentages else 0
        ...
        if max_concentration < 30:
            ...
            risk_level = RiskConcentration.ELEVATED
            recommendations = [f"Consider diversifying {dimension} concentration"]
        else:
            risk_level = RiskConcentration.CRITICAL
            recommendations = [f"High {dimension} concentration detected - immediate diversification recommended"]
    ...
    def generate_portfolio_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_premium": metrics.total_premium,
            "total_limits": metrics.total_limits,
            "average_score": metrics.weighted_avg_score,
            "tier_distribution": tier_dist,
            "risk_distribution": tier_dist,  # Same as tier_distribution
            "expected_loss_ratio": metrics.expected_loss_ratio,
            "expected_combined_ratio": metrics.expected_combined_ratio
        }
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": self.generate_portfolio_summary() if self.policies else {}
        }

    def get_tier_distribution(self) -> Dict[str, int]:
        """Get distribution of policies across risk tiers"""
        if not self.policies:
            return {f"tier_{i}": 0 for i in range(1, 6)}
