"""WC Inference Functions (V6/B2 depth-first build)
==================================================

Registers inference functions for every signal declared in
``coverages/wc/config.yaml``. Neutral scaffolds — each returns a
SignalResult with score=500 and proxy-tier-scaled confidence. Real
bodies land under Stage 6 (D-extractor depth — OSHA, state WC
boards, NCCI class codes).
"""
...
async def wc_004(entity_id: str, context: Any) -> SignalResult:
...
async def wc_028(entity_id: str, context: Any) -> SignalResult:
