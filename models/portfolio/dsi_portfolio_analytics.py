...
class DSIPortfolioAnalytics:
    ...
    def add_policy(self, policy_data: Dict[str, Any], pricing_result: Any):
        ...
        policy = {
            ...
            'company_name': pricing_result.company_name if hasattr(pricing_result, 'company_name') else pricing_result.institution_name,
            'data': policy_data,
    ...
    def calculate_portfolio_metrics(self) -> PortfolioMetrics:
        ...
        total_limits = sum(getattr(r, 'policy_limit', getattr(r, 'recommended_limit', 0)) 
    ...
    def _analyze_dimension(self, dimension: str) -> ConcentrationAnalysis:
        ...
        for key in concentrations:
            concentrations[key]['percentage'] = (concentrations[key]['premium'] / total_premium) * 100
        ...
        recommendations = []
        for key, data in sorted(concentrations.items(), key=lambda x: x[1]['percentage'], reverse=True)[:3]:
    ...
    def generate_alerts(self) -> List[PortfolioAlert]:
        ...
        if metrics.tier_4_count + metrics.tier_5_count > len(self.results) * 0.25:
            poor_policies = [p['id'] for p in self.policies 
        ...
        for conc in concentrations:
            if conc.risk_level in [RiskConcentration.CONCERNING, RiskConcentration.CRITICAL]:
                top_concentration = max(conc.concentrations.items(), key=lambda x: x[1]['percentage'])
                alerts.append(PortfolioAlert(
                    alert_id=f"CONC_{conc.concentration_type[:3].upper()}",
                    severity="critical" if conc.risk_level == RiskConcentration.CRITICAL else "warning",
                    ...
                    affected_policies=[],
                    recommended_action=conc.recommendations[0] if conc.recommendations else "Diversify portfolio"
                ))
    ...
    def compare_to_benchmark(self, benchmark: Dict[str, float]) -> Dict[str, Dict[str, float]]:
        ...
        comparison = {
            ...
            'combined_ratio': {
                ...
                'difference': metrics.expected_combined_ratio - benchmark.get('combined_ratio', 0.98),
    ...
    def generate_portfolio_report(self, output_format: str = 'dict') -> Union[Dict, pd.DataFrame, str]:
        ...
        report = {
            ...
            'concentrations': {
                conc.concentration_type: {
                    ...
                    'top_3': dict(sorted(conc.concentrations.items(), 
...
class DSIUnifiedAPI:
    ...
    def _result_to_dict(self, result: Any) -> Dict[str, Any]:
        ...
        return {
            'company_name': getattr(result, 'company_name', getattr(result, 'institution_name', '')),
