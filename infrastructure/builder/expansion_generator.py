...
class SignalCodeGenerator:
    ...
    def _generate_inference_functions(self, signals: List[SignalSpec]) -> str:
        ...
        extractor_module = (
            f"signal_architecture.signals.extractors.stubs."
            f"{self.coverage_dir}.{phase_name}_extractors"
        )
        aggregator_module = (
            f"signal_architecture.signals.aggregators.implementations."
            f"{self.coverage_dir}.{phase_name}_aggregators"
        )
        ...
        # Import all extractors and aggregators
        extractor_imports = []
        aggregator_imports = []
        for sig in signals:
            ext_class = self._to_class_name(sig.id) + "Extractor"
            agg_class = self._to_class_name(sig.id) + "Aggregator"
            extractor_imports.append(ext_class)
            aggregator_imports.append(agg_class)

        lines.append(f"from {extractor_module} import (")
        for imp in extractor_imports:
            lines.append(f"    {imp},")
        lines.append(")")
        lines.append("")
        lines.append(f"from {aggregator_module} import (")
        for imp in aggregator_imports:
            lines.append(f"    {imp},")
        lines.append(")")
        lines.append("")
        lines.append("")
        ...
        for i, sig in enumerate(signals, 1):
            ...
            if sig.signal_type == SignalType.CATEGORICAL:
                lines.append(
                    f"    return await _run_categorical("
                    f'"{sig.id}", {ext_class}(), {agg_class}(), entity_id, context)'
                )
            else:
                lines.append(
                    f"    return await _run_pipeline("
                    f'"{sig.id}", {ext_class}(), {agg_class}(), entity_id, context)'
                )
