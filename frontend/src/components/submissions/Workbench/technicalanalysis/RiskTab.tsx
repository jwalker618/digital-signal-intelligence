  ...
  return (
    <div className="w-full no-scrollbar animate-in fade-in duration-500 pb-12">
      ...
      <div className="pt-2 pb-2">
        ...
        <div className="
          overflow-x-hidden
          bg-generate-analysis shadow-sm 
          pt-generate-gap pb-4"
          >
          {hasTierMargin ? (
                  ...
                  {tierBands.map((band: any, idx: number) => {
                    ...
                    return (
                      <div key={band.tier_id} className="relative text-wrap">
                        <div className={`h-generate-gap rounded ${isCurrent ? 'bg-generate-contrast-background/30 border-4 border-generate-outline/50' : 'bg-generate-contrast-background/30 border border-generate-outline/50'}`}>
                          ...
                          {isCurrent && marginPct != null && (
                            <div className="
                              ...
                              <div className="
                                absolute
                                text-right
                                bottom-generate-padicon
                                text-xs
                                font-bold 
                                text-generate-selected whitespace-nowrap">
                        ...
                        <div className="flex justify-between mt-1 text-xs">
                          ...
                          <span className={`${isCurrent ? 'font-bold text-generate-selected' : 'text-xs'}`}>T{band.tier_id} ({band.label})</span>
                          <span>{band.bands?.min ?? ''}</span>
                        </div>
                        
                        <div className="text-center mt-0.5">
                          <span className={`text-xs ${band.action?.toLowerCase().includes('approve') ? 'text-generate-approve' : band.action?.toLowerCase().includes('decline') ? 'text-generate-decline' : 'text-generate-refer'}`}>
                            {band.action}
                          </span>
                        </div>
                      </div>
                ...
                <div className="border-t border-generate-outline/20 pt-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {improvementPaths.map((path: any) => (
                      <div key={path.tier_id} className="border border-generate-outline/15 rounded-lg p-3 text-wrap hover:bg-generate-background/10 transition-colors">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-bold">Tier {path.tier_id} — {path.label}</span>
                          <span className={`text-xs font-bold px-2 py-0.5 rounded ${path.action?.toLowerCase().includes('approve') ? 'bg-generate-approve/10 text-generate-approve' : 'bg-generate-refer/10 text-generate-refer'}`}>
                            {path.action}
                          </span>
                        </div>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs opacity-50">Score range: {path.target_range}</span>
                          <span className="text-sm font-black text-generate-approve">+{path.pts_needed} pts</span>
                        </div>
                        {topLevers.length > 0 && (
                          <div className="text-[10px] opacity-50 border-t border-generate-outline/10 pt-1.5 mt-1.5">
        ...
        <div className="overflow-x-auto">
          ...
          {isFetchingRiskSignals ? (
            ...
            <div className="w-full">
              {/* Column headers */}
              <div className="grid grid-cols-[1fr_70px_70px_90px_60px_120px_120px] gap-0 text-[11px] underline opacity-70 px-generate-pad py-2">
                ...
                return (
                  <div key={groupId}>
                    ...
                    <div
                      ...
                      className={`grid grid-cols-[1fr_70px_70px_90px_60px_120px_120px] gap-0 px-generate-pad py-2.5 border-t border-generate-outline/20 cursor-pointer hover:bg-generate-background/20 transition-colors ${hasConditions ? 'border-l-2 border-l-generate-refer' : ''}`}
                    >
                      <div className="flex items-center gap-2">
                        {isExpanded ? <ChevronDown className="w-3.5 h-3.5 shrink-0" /> : <ChevronRight className="w-3.5 h-3.5 shrink-0" />}
                        <span className="font-bold text-sm">{groupId}</span>
                        {groupConditions.length > 0 && (
                          <span className="text-[10px] bg-generate-refer/10 text-generate-refer px-1.5 py-0.5 rounded font-bold">
                      ...
                      <span className="text-center">
                        <span className={`text-xs ${(gs.coverage_ratio ?? 0) >= 1 ? 'text-generate-approve' : (gs.coverage_ratio ?? 0) >= 0.5 ? 'text-generate-refer' : 'text-generate-decline'}`}>
                      ...
                      <span className="text-right text-xs text-wrap">
                        {lossWeight > 0 ? (
                          <span>
                            ...
                            {currentLossBand && (
                              <span className="block text-[9px] font-bold uppercase text-generate-info">→ {currentLossBand.label} ({currentLossBand.frequency_modifier}x)</span>
                            )}
                          </span>
                      ...
                      <span className="text-right text-xs text-wrap">
                        {expWeight > 0 ? (
                          <span>
                            ...
                            {currentExpBand && (
                              <span className="block text-[9px] font-bold uppercase text-generate-info">→ {currentExpBand.label} ({currentExpBand.modifier}x)</span>
                            )}
                          </span>
                        ) : <span className="opacity-20">–</span>}
                      </span>
                    ...
                    {isExpanded && groupConditions.length > 0 && (
                      <div className="ml-8 mr-generate-pad mb-1">
                    ...
                    {isExpanded && signals.map((sig: any, sidx: number) => {
                      ...
                      return (
                        <div
                          key={`${sig.code}-${sidx}`}
                          className={`grid grid-cols-[1fr_70px_70px_90px_60px_120px_120px] gap-0 px-generate-pad py-1.5 bg-generate-background/10 hover:bg-generate-background/20 transition-colors ${sigConditions.length > 0 ? 'border-l-2 border-l-generate-refer' : ''}`}
                        >
                          <div className="flex items-center gap-2 pl-6">
                            <span className="text-sm">{sig.code}</span>
                            {sigConditions.length > 0 && <AlertTriangle className="w-3 h-3 text-generate-refer shrink-0" />}
              ...
              {(() => {
                ...
                return (
                  <div className="grid grid-cols-[1fr_70px_70px_90px_60px_120px_120px] gap-0 px-generate-pad py-3 border-t-2 border-generate-outline/30 bg-generate-background/20">
                    ...
                    <span className="text-right text-xs text-wrap">
                      {currentLossBand ? (
                        <span className="font-bold text-generate-info uppercase">{activeVersion?.loss_propensity_band?.replace(/_/g, ' ')} → {formatNumber(activeVersion?.loss_combined_modifier, 3)}x</span>
                      ) : <span className="opacity-30">–</span>}
                    </span>
                    <span className="text-right text-xs text-wrap">
                      {currentExpBand ? (
                        <span className="font-bold text-generate-info uppercase">{activeVersion?.exposure_band_label} → {formatNumber(activeVersion?.exposure_modifier, 3)}x</span>
                      ) : <span className="opacity-30">–</span>}
                    </span>
                  </div>
                );
              ...
              {/* Tier result summary */}
              <div className="px-generate-pad py-2 text-xs opacity-60 text-wrap">
                Composite {formatNumber(currentScore, 1)} → <span className="font-bold text-generate-selected">Tier {activeVersion?.final_tier} ({activeVersion?.tier_label})</span>
          ...
          {conditionTypeSummary.length === 0 ? (
            ...
            <div>
              {conditionTypeSummary.map(({ type, count, modifierCount, aggregateModifier }) => {
                ...
                return (
                  <div key={type}>
                    <div onClick={() => toggleGroup(`cond_${type}`)} className="flex items-center justify-between px-generate-pad py-2.5 border-b border-generate-outline/20 cursor-pointer hover:bg-generate-background/20 transition-colors">
                      <div className="flex items-center gap-2">
                        {isExpanded ? <ChevronDown className="w-3.5 h-3.5 shrink-0" /> : <ChevronRight className="w-3.5 h-3.5 shrink-0" />}
                      ...
                      <div className="flex items-center gap-2">
                        {modifierCount > 0 && (
                          <span className="text-[10px] bg-generate-info/15 text-generate-info px-1.5 py-0.5 rounded font-bold">
                    ...
                    {isExpanded && conds.map((cond: any, cidx: number) => {
                      ...
                      return (
                        <div key={cidx} className="flex items-center justify-between px-generate-pad py-2 pl-8 bg-generate-background/10 border-b border-generate-outline/5 hover:bg-generate-background/20 transition-colors">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              {type === 'direct_query' && (
                                <span className={`text-xs font-bold ${cond.response === true || cond.response === 'yes' ? 'text-generate-approve' : cond.response === false || cond.response === 'no' ? 'text-generate-decline' : 'opacity-70'}`}>
