    ...
    def get_category_score(self, category: str) -> float:
        ...
        if category == "infrastructure":
            signals = [
                ...
                self.dnssec_implementation,
                self.spf_dmarc_dkim,
                self.web_application_firewall,
            ]
        elif category == "vulnerability":
            signals = [
                self.open_ports_score,
                self.outdated_software,
                self.known_vulnerabilities,
                self.exposed_databases,
                self.leaked_credentials,
                self.breached_history,
            ]
        elif category == "organizational":
            signals = [
                self.security_certifications,
                self.privacy_policy_quality,
                self.incident_response_plan,
                self.bug_bounty_program,
                self.security_team_visibility,
                self.security_blog_activity,
            ]
        elif category == "third_party":
            signals = [
                self.vendor_security_standards,
                self.supply_chain_transparency,
                self.cloud_provider_quality,
                self.third_party_integrations,
                self.data_processor_agreements,
            ]
        elif category == "behavioral":
            signals = [
                self.patch_discipline,
                self.security_investment,
...
class CyberInsurancePricingModel:
    ...
    def calculate_cyber_maturity_modifier(
        self, signals: CyberSecuritySignals
    ) -> Tuple[float, float]:
        """
    ...
    def calculate_vulnerability_modifier(
        self, signals: CyberSecuritySignals
    ) -> Tuple[float, float]:
        """
    ...
    def estimate_breach_probability(
        self, composite_score: float, industry: IndustryVertical, prior_incidents: int
    ) -> float:
    ...
    def estimate_expected_loss(
        self, company: CyberCompanyProfile, breach_probability: float
    ) -> float:
    ...
    def recommend_policy_structure(
        self, company: CyberCompanyProfile, expected_loss: float
        ...
        recommended_limit = min(
            expected_loss * 2.5, revenue_mm * 0.15 * 1_000_000  # 15% of revenue max
        )
    ...
    def calculate_confidence(self, signals: CyberSecuritySignals) -> float:
        ...
        all_signals = [
            signals.ssl_certificate,
            signals.tls_version,
            signals.security_headers,
            signals.dnssec_implementation,
            signals.spf_dmarc_dkim,
            signals.web_application_firewall,
            signals.open_ports_score,
            signals.outdated_software,
            signals.known_vulnerabilities,
            signals.exposed_databases,
            signals.leaked_credentials,
            signals.breached_history,
            signals.security_certifications,
            signals.privacy_policy_quality,
            signals.incident_response_plan,
            signals.bug_bounty_program,
            signals.security_team_visibility,
            signals.security_blog_activity,
            signals.vendor_security_standards,
            signals.supply_chain_transparency,
            signals.cloud_provider_quality,
            signals.third_party_integrations,
            signals.data_processor_agreements,
            signals.patch_discipline,
            signals.security_investment,
            signals.employee_training,
            signals.mfa_adoption,
            signals.backup_procedures,
            signals.monitoring_capabilities,
        ]
    ...
    def generate_recommendation(
        ...
        if composite_score >= 800 and vulnerability_score >= 85 and confidence >= 0.85:
            ...
            conditions = [
                ...
                "MFA implementation required within 90 days",
            ]
            ...
            conditions = [
                "Complete cyber security questionnaire",
            ...
            conditions = [
                "Comprehensive security assessment required",
                "Penetration test mandatory",
                "Remediation plan for identified vulnerabilities",
                "Higher deductible/sublimits",
                "Quarterly security reviews",
                "MFA mandatory",
                "EDR/XDR solution required",
            ]

        return rec, reason, conditions

    def price(
        self, company: CyberCompanyProfile, requested_limit: Optional[float] = None
    ) -> CyberPricingResult:
        """
        ...
        industry_mod = self.industry_multipliers[company.industry.value]  # Use .value for enum
...
if __name__ == "__main__":
    ...
    moderate_healthcare_signals = CyberSecuritySignals(
        ...
        tls_version=80,
        security_headers=70,
        dnssec_implementation=60,
        spf_dmarc_dkim=75,
        web_application_firewall=65,
        open_ports_score=65,
        outdated_software=58,
        known_vulnerabilities=62,
        exposed_databases=70,
        leaked_credentials=65,
        breached_history=60,
        security_certifications=72,
        privacy_policy_quality=80,
        incident_response_plan=68,
        bug_bounty_program=0,
        security_team_visibility=70,
        security_blog_activity=45,
        vendor_security_standards=68,
        supply_chain_transparency=60,
        cloud_provider_quality=75,
        third_party_integrations=65,
        data_processor_agreements=78,
        patch_discipline=58,
        security_investment=65,
        employee_training=70,
        mfa_adoption=72,
        backup_procedures=75,
    ...
    poor_retail_signals = CyberSecuritySignals(
        ...
        tls_version=60,
        security_headers=45,
        dnssec_implementation=30,
        spf_dmarc_dkim=55,
        web_application_firewall=40,
        open_ports_score=45,
        outdated_software=38,
        known_vulnerabilities=42,
        exposed_databases=50,
        leaked_credentials=40,
        breached_history=35,
        security_certifications=40,
        privacy_policy_quality=50,
        incident_response_plan=35,
        bug_bounty_program=0,
        security_team_visibility=45,
        security_blog_activity=20,
        vendor_security_standards=42,
        supply_chain_transparency=38,
        cloud_provider_quality=60,
        third_party_integrations=45,
        data_processor_agreements=40,
        patch_discipline=35,
        security_investment=40,
        employee_training=38,
        mfa_adoption=45,
        backup_procedures=50,
        monitoring_capabilities=42,
    )
    ...
    for label, company in companies:
        ...
        print(
            f"  Premium as % of Limit: {(result.annual_premium / result.recommended_limit * 100):.2f}%"
        )
