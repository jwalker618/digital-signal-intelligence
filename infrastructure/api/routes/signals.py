...
async def override_signal(
    ...
    # ── b2. Record signal override as a submission note ───────────────────
    override_note = SubmissionNote(
        submission_id=old_model.submission_id,
        note=f"Signal override: {request.signal_code} changed from {original_score} to {request.audited_value}",
